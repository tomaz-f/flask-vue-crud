...
export default {
  ...
  methods: {
    ...
    toggleDeletebBookModal(book) {
      if (book) {
        this.deleteBookForm = book;
      }
      const body = document.querySelector("body");
      this.activeDeleteBookModal = !this.activeDeleteBookModal;
      if (this.activeDeleteBookModal) {
        body.classList.add("modal-open");
      } else {
        body.classList.remove("modal-open");
      }
    ...
    handleDeleteSubmit() {
      this.toggleDeleteBookModal(null);
      this.removeBook(this.deleteBookForm.id);
    },
    handleEditSubmit() {
